...
"""
    ...
    async def get_recent_messages(self, k: int = 3) -> list:
        """
        Get recent conversation messages for frontend history display.
        Returns a list of message dictionaries with 'role' and 'content' keys.
        """
        try:
            if not self.memory_id:
                logger.debug("No memory ID available for history retrieval")
                return []
            ...
            if self.memory_client:
                try:
                    # Get conversation history using AgentCore MemoryClient
                    memory_response = await loop.run_in_executor(
                        None,
                        lambda: self.memory_client.get_memory(
                            memory_id=self.memory_id,
                            actor_id=self.actor_id,
                            session_id=self.session_id,
                            branch_name=self.branch_name,
                            max_messages=k * 2,  # Get more to account for user/agent pairs
                        ),
                    )

                    messages = []
                    if memory_response and "messages" in memory_response:
                        # Process the memory response to extract user/agent messages
                        for msg in memory_response["messages"][-k * 2:]:  # Get last k pairs
                            if isinstance(msg, dict):
                                ...
                                # Handle tuple format (content, role)
                    ...
                    return messages[-k * 2:] if messages else []  # Return last k pairs

                except Exception as e:
                    logger.warning(f"AgentCore memory retrieval failed: {str(e)}")
            ...
            if self._bedrock_agentcore:
                try:
                    ...
                    if response and "messages" in response:
                        for msg in response["messages"][-k * 2:]:
                            if isinstance(msg, dict):
                                content = msg.get("content", "")
                                role = msg.get("role", "")
                    ...
                    logger.info(f"Retrieved {len(messages)} messages from boto3 fallback")
                    return messages[-k * 2:] if messages else []

                except Exception as e:
                    logger.warning(f"boto3 memory retrieval failed: {str(e)}")

            logger.debug("No memory client available for history retrieval")
            return []
