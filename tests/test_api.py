...
class TestCustomEnvisionAgent:
    ...
    def test_system_prompt_contains_envision_context(self, agent):
        ...
        envision_terms = ["Envision", "sustainable infrastructure", "framework"]

        system_prompt_lower = system_prompt.lower()
        for term in envision_terms:
            assert (
                term.lower() in system_prompt_lower
            ), f"System prompt missing '{term}'"
    ...
    async def test_memory_content_retrieval(self, agent):
        ...
        agent._load_conversation_history = AsyncMock(
    ...
    async def test_knowledge_base_retrieval(self, agent):
        ...
        agent._converse_with_retry = AsyncMock(
            return_value={
                "output": {
                    "message": {
                        "content": [
                            {"text": "Test response about sustainable infrastructure"}
                        ]
                    }
                }
            }
    ...
    def test_extract_text_from_response(self, agent):
        ...
        # Test dict response
        dict_response = {"content": [{"text": "Dict text response"}]}
...
class TestAgentIntegration:
    ...
    async def test_agent_memory_integration(self, mock_agent):
        ...
        # Mock memory methods
        mock_agent._load_conversation_history = AsyncMock(
            return_value="Previous context"
        )
        ...
        mock_agent._store_conversation_turn.assert_called_once_with(
            "Hello", "Hi there!"
        )
...
class TestLambdaIntegration:
    ...
    def test_lambda_environment_variables(self):
        ...
        expected_env_vars = ["AGENT_ARN", "AWS_DEFAULT_REGION"]
